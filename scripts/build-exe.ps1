# WinMole - Standalone EXE Builder
...
$exeName = "winmole-$Version-x64.exe"
...
function Show-BuildHelp {
    Write-Host ""
    Write-Host "WinMole Standalone EXE Builder" -ForegroundColor Cyan
    Write-Host ""
...
$launcherPath = Join-Path $releaseDir "winmole-launcher.ps1"

# Create a self-contained launcher that embeds the main script
$launcherContent = @"
#Requires -Version 5.1
# WinMole - Standalone Launcher
# Version: $Version
...
`$script:WINMOLE_VER = "$Version"
`$script:WINMOLE_BUILD = "$(Get-Date -Format 'yyyy-MM-dd')"
...
if (`$isCompiled) {
    ...
    Write-Host ""
    Write-Host "Download from: https://github.com/bhadraagada/winmole/releases" -ForegroundColor Gray
    exit 1
}

# If running as script, delegate to main winmole.ps1
`$winmoleScript = Join-Path `$PSScriptRoot "winmole.ps1"
if (Test-Path `$winmoleScript) {
    & `$winmoleScript `$Command `$CommandArgs
} else {
    Write-Host "Error: winmole.ps1 not found in `$PSScriptRoot" -ForegroundColor Red
    exit 1
}
"@
...
try {
    ...
    $ps2exeParams = @{
        ...
        outputFile = $exePath
        noConsole = $false
        title = "WinMole - Windows System Maintenance"
        description = "Deep clean and optimize your Windows system"
        company = "WinMole Project"
        product = "WinMole"
...
Write-Host ""
Write-Host "Note: This is a launcher EXE that requires the full WinMole distribution" -ForegroundColor Yellow
